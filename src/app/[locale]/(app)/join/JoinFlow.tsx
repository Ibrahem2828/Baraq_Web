 ...
 */
export function JoinFlow({ initialToken }: { initialToken: string | null }) {
  ...
  const linkToken = dismissedLink ? null : initialToken;
      ...
      <div className="flex flex-col gap-8">
        ...
        <section>
          ...
          {memberships.isPending ? (
            <LoadingState />
          ) : !memberships.data || (memberships.data.organizations.length === 0 &&
              memberships.data.classes.length === 0 &&
              memberships.data.join_requests.length === 0) ? (
            <EmptyState title={t("join.noMemberships")} />
          ) : (
            <StaggerIn className="flex flex-col gap-3">
              {/* A learner's place has three parts and they mean different
                  things: the school, the classes inside it, and anything
                  still waiting on a teacher. */}
              {memberships.data.organizations.map((entry) => (
                <StaggerItem key={entry.organization.public_id}>
                  <Card className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      <strong className="block">{entry.organization.name}</strong>
                      <small className="opacity-70">
                        {entry.organization.organization_type === "institute"
                          ? t("join.institute")
                          : t("join.school")}
                      </small>
                    </div>
                    <Badge variant={entry.status === "active" ? "success" : "warning"}>
                      {entry.status === "active" ? t("join.statusActive") : t("join.statusPending")}
                    </Badge>
                  </Card>
                </StaggerItem>
              ))}

              {memberships.data.classes.map((entry) => (
                <StaggerItem key={entry.classroom.public_id}>
                  <Card className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      <strong className="block">{entry.classroom.name}</strong>
                      <small className="opacity-70">
                        {t("join.classLabel")} · {entry.organization.name}
                      </small>
                    </div>
                    <Badge variant={entry.status === "active" ? "success" : "warning"}>
                      {entry.status === "active" ? t("join.statusActive") : t("join.statusPending")}
              ...
              {memberships.data.join_requests
                .filter((request) => request.status === "pending")
                .map((request) => (
                  <StaggerItem key={request.public_id}>
                    <Card className="flex flex-wrap items-center justify-between gap-3">
                      <div>
                        <strong className="block">{request.organization.name}</strong>
                        <small className="opacity-70">
                          {request.classroom
                            ? `${t("join.classLabel")}: ${request.classroom.name}`
                            : t("join.pendingBody")}
                        </small>
                      </div>
                      <Badge variant="warning">{t("join.statusPending")}</Badge>
                    </Card>
                  </StaggerItem>
                ))}
            </StaggerIn>
