...
| AccountDeletionScreen | Settings destructive-action dialog | IMPLEMENTED_VERIFIED | Uses authenticated `DELETE /users/me/` through the BFF. On success the BFF clears HttpOnly auth cookies; the backend anonymizes and soft-deletes the account and revokes outstanding tokens. |
